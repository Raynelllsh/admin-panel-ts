...
export function useAdminData() { // Removed explicit return type temporarily to avoid interface errors until you update types.ts
  // --- GLOBAL STATE ---
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      reqs.sort((a, b) => new Date(b.submitTime).getTime() - new Date(a.submitTime).getTime());
      setRequests(reqs);
    });
    ...
    const fetchData = async () => {
      try {
        ...
        const uniqueRounds = Array.from(
          new Set(
            loadedCourses.map((c) => c.path?.round || "round001")
          )
        ).sort();
  ...
  const addStudentToLesson = async (
    ...
    const isAlreadyIn = lesson.students.some((id) => String(id) === String(studentId));
    if (isAlreadyIn) {
  ...
  const handleRequest = async (request: LessonChangeRequest, action: "approve" | "reject") => {
    try {
      if (action === "reject") {
